...
version: 1.0
date: "2025-Sep-08"
date-modified: "2025-September-17"
type: website
output-dir: docs
editor: visual
...
The following is a preliminary analysis of the randomised trial described in the published protocol [@fatima2022]. The aims of this analysis were to;

::: {#objectives}
> The primary objective of this study is to determine, in patients electing to undergo surgical intervention for propulsive metatarsalgia, whether wedge-cut osteotomy compared to flat-cut Weil (with or without adjunct pro- cedures) is associated with increased pain relief during activities of daily living up to 12 months postoperatively. The secondary objective of this study is to determine whether a lower incidence of procedure-specific compli- cations (floating toe or stiffness) at up to 12 months of follow-up is associated with the wedge-cut procedure, compared to the flat-cut Weil.
:::

## 

The primary objective of this study is to determine, in patients electing to undergo surgical intervention for propulsive metatarsalgia, whether wedge-cut osteotomy

compared to flat-cut Weil (with or without adjunct pro- cedures) is associated with increased pain relief during

activities of daily living up to 12 months postoperatively. The secondary objective of this study is to determine

whether a lower incidence of procedure-specific compli- cations (floating toe or stiffness) at up to 12 months of

follow-up is associated with the wedge-cut procedure, compared to the flat-cut Weil.

## Reporting

The methods and results are reported with a focus on primary and secondary outcomes. Additional context as per CONSORT [@Schulz2010] has been reserved for a more comprehensive report of the study. It should be noted that the CONSORT statement has since been updated [@Hopewell2025] and the final version of the study analysis will likely need to conform to the new statement, which may not align fully with the published protocol for this study.

## Preparation

The relevant packages required for the analysis were loaded in advance.
...
The packages in use for this analysis (so far) are outlined in @tbl-pkgcite.

## Authorisations

Access to the study handover file was pre-authorised using the *gargle* and *googledrive* packages.

```{r}
#| label: authorisation-1
#| echo: false
...
A series of functions were specified to call later in the file for processing data imports. A general text cleaning function was constructed to apply during first import of raw data files, built on *tidyverse* and *stringr*. The *janitor* package was utilised to clean column names in the resulting dataframe.
...
A function to replace abbreviated terms in complications and descriptions was constructed.

```{r}
#| label: replace-term-func
...
Data was sourced directly from the [study handover file](https://docs.google.com/spreadsheets/d/1mrG1Oxv3oQbalTlh6xr3kGMZcpOz5m76unqbe0eu_uc/edit?usp=sharing), which was combined from the SOFARI registry snapshot and the study masterfile used for tracking during the recruitment and data collection phase. Patient and treatment information were entered into the database through the registry interface and compiled into a data cube (snapshot) every quarter. Complications and adverse events were captured into an online form (QuestionPro, USA) and linked using record identifier codes. Patient-reported outcomes were collected from the patient through electronic communication (sms, email) of a form link specific to baseline assessment and captured into an online form (QuestionPro, USA) for each questionnaire of interest. An additional link was sent specific for the study (DASS questionnaire) at the scheduled followups. PROMs data were also linked back to patient and treatment infromation using record identifier codes.

## Data Import and Preparation

Data was retrieved and formatted using *googlesheets4* to retrieve live database tables. Text and code output were integrated using the *epoxy* package. Source files were specified and stored as global variables to call on in further functions.
...
The self-assessed comorbidity score [@Sangha2003] had to be added to the dataset by calculating from individual responses included in the registry snapshot. The scores were then added to the analysis master table.
...
The DASS-21 responses were also scored as per [@henry2005] with the final score for each subscale (Depression, Anxiety and Stress) doubled to align with the full DASS score distributions.
...
Additional pain management strategies (medications and non-pharmacological measures) were extracted at each timepoint and compiled for further analysis.
...
Intraoperative data was captured via electronic form and compiled for futher analysis by condensing secondary pathology and adjunct procedures responses into one column respectively.
...
Adverse events were processed by separating events into planned removal of k-wires versus unexpected events such as infection, vein thrombosis or condition recurrence. Text processing was used to replace abbreviated terms, standardise phrasing and filter duplicate entries for the same treatment record.

```{r}
#| label: adverse-events-1
...
The free-text describing the nature of the complication or adverse event was pre-processed using *tidytext* to split into word tokens and remove stop words.

```{r}
#| label: adverse-events-3
...
Terms with less than four characters were extracted and reproduced in an external file for manual spelling of abbreviations.

```{r}
#| label: adverse-events-4
...
Terms identified for replacement were written to an external file for manual input of expanded definitions.

```{r}
#| label: adverse-events-5
#| eval: false
...
The description of the complication for each entry was recompiled from the cleaned text and joined back to the master table. Duplicate entries were filtered from the dataset.

```{r}
#| label: adverse-events-8
...
The complication descriptions were manually reviewed and categorised based on the responses retrieved. These were - infection, malalignment, deep vein thrombosis and circulation issues, recurrent or persistent forefoot symptoms, with the remaining cases classified as other (e.g. subsequent ankle fracture and orif).

```{r}
#| label: adverse-events-9
# Finalise complications master
# 
...
MasterComplic3 <- MasterComplic1c |> dplyr::mutate(
  ...
  AnyComplication = if_any(c(Infection, Malalignment, DVT_Circulation, Symptoms, Other))
)
...
A dataframe was prepared to derive the CONSORT flow chart as per the protocol [@fatima2022].
...
The flowchart @fig-consort shows dropouts for Group A which included one case that presented with new pathology that was followed in a subsequent treatment record and two patients that opted out of receiving PROMs reminders from the registry system.

## Finalise Master Table

The master table of the eligible cases for analysis were compiled by filtering the consort flowchart input dataframe for those that were eligible for final followup as part of the intention-to-treat analysis (mITT).
...
MasterTable <- CONSORTFlow |> dplyr::filter(
  ...
  Sex2 = if_else(
    ...
    MasterComplic3 |> dplyr::select(
      ...
      OccurDelay,
      ComplicationTreatment,
      Infection:AnyComplication
    ) |> group_by(TreatmentID) |> slice_min(
      OccurDelay
    ) |> ungroup(),
    by = "TreatmentID"
  ) |> dplyr::mutate(
    across(c(Infection:AnyComplication), ~ replace_na(.x, FALSE))
  )
...
Characteristics of cases in both groups was described in a table using *gtsummary*. Age, sex, comorbidity burden, smoking history and baseline PROMs were combined with facility, secondary pathology.
...
The descriptives table @tbl-summary shows that nearly all cases presented with a low comorbidity burden on average, one or two metatarsals involved as well as secondary pathology.

# Missingness

The magnitude and pattern of missingness was assessed within the input dataset for the primary outcome analysis using the *naniar* package.
...
FigMiss <- vis_miss(MasterAnalysis)

knitr::knit_print(FigMiss)

```

The missingness figure @fig-missingness identified a block pattern in smoking history and it was dropped from the analysis. The remaining variables showed missing-at-random patterns across the cohort.

Multiple imputation with chained equations [@mice], implemented in the *mice* package was selected to address the missingness across the dataset in preparation for the primary analysis. The process was initialised by declaring the methods and sequence of imputation to be followed based on the known relationships between variables.
...
Imputations were performed over 10 iterations for a total of 20 imputed datasets, prior to pooling for inclusion in the linear model of the primary analysis of FAOS Pain scores at 12months.

```{r}
#| label: multiple-imputation-2
...
A linear model was constructed from the pooled imputation data with FAOS Pain subscore as the outcome and Group adjusted by the baseline FAOS score, age, sex and the comorbidity score.

```{r}
#| label: primary-analysis-1
...
```{r}
#| label: tbl-primary
#| tbl-cap: "Adjusted comparison between groups for FAOS Pain at 12 month followup"
...
The model summary @tbl-primary showed no significant difference between groups (P = 0.6) and a small effect (2 points on the FAOS scale) which is well below the MCID of the FAOS overall [@desai2019].
...
```{r}
#| label: fig-model-predicted
#| fig-cap: "Model predicted FAOS-Pain at 12 months compared between groups"

FAOSPlot <- ggplot(data = FAOSPredict, mapping = aes(y = Group, x = FAOS_Pain_TotalScore_12months)) + 
  stat_slabinterval(aes(thickness = after_stat(pdf*n)), scale = 0.7) +
  stat_dotsinterval(side = "bottom", scale = 0.7, slab_linewidth = NA) +
  scale_fill_brewer(palette = "Set2") +
  ggtitle('FAOS Predicted by Group')

knitr::knit_print(FAOSPlot)
```

Visualisation of the model predictions @fig-model-predicted show the small differences in the average scores between groups and the non-normal distributions of the responses.

```{r}
#| label: tbl-modelpredicted
#| tbl-cap: "Model predicted FAOS at 12 months compared between groups"

TablePredict <- gtsummary::tbl_summary(
  FAOSPredict,
  include = c(
    FAOS_Pain_TotalScore_12months,
    Group
...
knitr::knit_print(TablePredict)


```

The model-predicted summary statistics in FAOS Pain at 12months @tbl-modelpredicted showed no difference between groups.

# Secondary Analysis - Complications

The incidence of complications and reoperations was compared between groups in a summary table with univariate comparisons performed on each outcome.
...
No significant differences were observed @tbl-complications for any outcome associated with complications or reoperations.

# Interpretation

The osteotomy technique utilised has no discernible effect on pain scores returned at 12months followup, or complication incidence at up to 12months followup. Choice of technique can be made by the operating surgeon based on factors other than patient outcomes.
